...
export default function CategoryListItem({category}:{category:CategoryCountsList}){
    ...
    return (
        ...
        <div className="relative md:w-[30%] md:h-[260px] group bg-white rounded-lg" >
            <Link className="absolute h-full w-full top-0 left-0 z-40" href={`https://e-commerce-frontend-three-mu.vercel.app/category/${category.id}`}></Link>
        ...
        <div className="infos md:w-[70%] flex flex-col gap-4">
            <Link href={`https://e-commerce-frontend-three-mu.vercel.app/category/${category.id}`} className="text-xl md:text-2xl font-semibold hover:text-[#E73F10] cursor-pointer line-clamp-1 ">{category.name}</Link>
