            ...
            categoryCounts.map((categoryCount:CategoryCountsList)=>{
                return (
                    <div key={categoryCount.id} className="flex gap-8" >
                        <Link style={{color:categoryCount.id == Number(param) ? '#E73F10' : 'black'}} href={`https://e-commerce-frontend-three-mu.vercel.app/category/${categoryCount.id}`} className="hover:!text-[#E73F10]" ><p className="text-lg font-medium">{categoryCount.name}</p></Link>
